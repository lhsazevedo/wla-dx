...
int main(int argc, char *argv[]) {
  ...
  if (verbose_mode == ON) {
    ...
    for (index = 0; index < romsize; index++) {
      if (rom_usage[index] == 0 && x == 0) {
        x = 1;
        y = index;
      }
      else if (rom_usage[index] != 0 && x == 1) {
        if (y == (index - 1))
    ...
    if (output_mode == OUTPUT_ROM) {
      for (index = 0, y = 0; index < romsize; index++) {
    ...
    /* program file data */
      ...
      for (index = program_start, y = 0; index < program_end; index++) {
        if (rom_usage[index] == 0)
          y++;
      }
    ...
    if (smc_status != 0) {
      ...
      index = file_header_size + file_footer_size + 512;
    }
    else
      index = file_header_size + file_footer_size;
...
int parse_and_set_libdir(char *c, int contains_flag) {
  ...
  for (index = 0; index < MAX_NAME_LENGTH && *c != 0; index++, c++)
    n[index] = *c;
  n[index] = 0;
