...
int listfile_block_write(FILE *file_out, struct section_def *section) {
  ...
  for (index = 0; index < section->listfile_items; index++) {
    fprintf(file_out, "%c", section->listfile_cmds[index]);
    if (section->listfile_cmds[index] == 'k') {
      /* next line: line number, line length, skip bytes */
      d = section->listfile_ints[index*3 + 0];
      WRITEOUT_D;
      d = section->listfile_ints[index*3 + 1];
      WRITEOUT_D;
      d = section->listfile_ints[index*3 + 2];
      WRITEOUT_D;
    }
    else if (section->listfile_cmds[index] == 'f') {
      /* next file: file name id */
      d = section->listfile_ints[index*3 + 0];
      WRITEOUT_D;
    }
    else {
      fprintf(stderr, "LISTFILE_BLOCK_WRITE: Unknown command '%c'. Internal error. Only known commands are 'k' and 'f'.\n", section->listfile_cmds[index]);
      return FAILED;
    }
  }
